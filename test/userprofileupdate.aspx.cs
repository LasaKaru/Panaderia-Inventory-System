...
namespace Panaderia.test
{
    public partial class userprofileupdate : System.Web.UI.Page
    {
        ...
        protected void LoadData()
        {
            ...
            string query = "SELECT txtUserID, txtUserName, txtDefaultStore, txtUserGroup, txtUserPassword, txtUserStatus, ddlUserStatus FROM [Panaderia].[dbo].[MF_User_Profile]";
        ...
        private void SaveOrUpdateData()
        {
            ...
            {
                // Insert logic
                query = "INSERT INTO MF_User_Profile (txtUserID, txtUserName, txtDefaultStore, ddlDefaultStore, txtUserGroup, txtUserPassword, txtUserStatus) VALUES (@txtUserID, @txtUserName, @txtDefaultStore, @ddlDefaultStore, @txtUserGroup,  @txtUserPassword, @txtUserStatus)";
            }
            else if (action.Equals("Update"))
            {
                // Update logic
                query = "UPDATE MF_User_Profile SET txtUserID  = @txtUserID, txtUserName  = @txtUserName, txtDefaultStore = @txtDefaultStore, ddlDefaultStore = @ddlDefaultStore , txtUserGroup = @txtUserGroup, txtUserPassword = @txtUserPassword, txtUserStatus  = @txtUserStatus WHERE txtUserID  = @txtUserID";
            }
            else
            {
            ...
            using (SqlConnection connection = new SqlConnection(connectionString))
            {
                using (SqlCommand command = new SqlCommand(query, connection))
                {
                    ...
                    //command.Parameters.AddWithValue("@txtUserStatus", txtUserStatus.Text);
                    command.Parameters.AddWithValue("@txtUserID", txtUserID.Text);
                    command.Parameters.AddWithValue("@txtUserName", txtUserName);
                    //command.Parameters.AddWithValue("@txtDefaultStore", txtDefaultStore.Text);
                    command.Parameters.AddWithValue("@ddlDefaultStore", ddlDefaultStore.Text);
                    command.Parameters.AddWithValue("@txtUserGroup", ddlDefaultStore.Text);
                    command.Parameters.AddWithValue("@txtUserPassword", txtUserPassword.Text);
                    //command.Parameters.AddWithValue("@txtUserStatus", txtUserStatus.Text);
                    ...
                    {
                        // Add parameter for the record ID when updating
                        //command.Parameters.AddWithValue("@RecordId", /* Get the record ID to update */);
                    ...
                    try
                    {
                        ...
                        // If successful, update the UI or do any additional tasks
                        // ...

                        // Set hidden field to indicate data has been saved or updated
                        hdnDataSaved.Value = "true";
                    }
                    catch (Exception ex)
                    {
                        ...
                        // Optionally, set hidden field to indicate data has not been saved
        ...
        private void SaveOrUpdateData(string action)
        {
