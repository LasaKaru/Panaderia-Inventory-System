        ...
        protected void btnSave_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                using (SqlConnection connection = new SqlConnection(connectionString))
                {
                    ...
                    string insertMainQuery = @"INSERT INTO [dbo].[Inv_Invoice_credit]
...
(@CompanyID,@IPS_Date,@Branch,@TxnType,@Number,@User,@cus_nu,@code,@cus_name,@Amount,@CustomerReferance,@Discount,@txtComments,@SO_nu)";
                    ...
                    using (SqlCommand cmdMain = new SqlCommand(insertMainQuery, connection))
                    {
                        ...
                        cmdMain.Parameters.AddWithValue("@code", txtcusID.Text);
                        cmdMain.Parameters.AddWithValue("@cus_name", txtCusname.Text);
                        cmdMain.Parameters.AddWithValue("@Amount", txtamount.Text);
                        cmdMain.Parameters.AddWithValue("@CustomerReferance", txtaReferance.Text);
                        cmdMain.Parameters.AddWithValue("@Discount", txtDiscount.Text);
                        cmdMain.Parameters.AddWithValue("@txtComments", txtComments.Text);
                        cmdMain.Parameters.AddWithValue("@SO_nu", txtSONum.Text);
                    ...
                    foreach (DataRow row in dt.Rows)
                    {
                        string insertDetailQuery = "INSERT INTO sales_invoice_footer (ItemCode, Description, Price, PSize, Packs, Nos, Discount, Amount, GrandTotal) " +
