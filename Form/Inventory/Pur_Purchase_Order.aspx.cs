    ...
    public partial class Pur_Purchase_Order : System.Web.UI.Page
    {
        ...
        /*protected void btnSave_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                // Generate a unique ID for the entire bill
               ...
               (@CompanyID, @IPS_Date, @Branch, @TxnType, @Number, @User, @Code, @Sup_Name, @Amount, @SupplierReference, @Discount, @Comments,@Sup_nu)";
                    ...
                    foreach (DataRow row in dt.Rows)
                    {
                        string insertDetailQuery = "INSERT INTO purchase_order_footer1 (BillID, ItemCode, Description, Price, PSize, Packs, Nos, Discount, Amount, GrandTotal) " +
                                     "VALUES (@BillID, @ItemCode, @Description, @Price, @PSize, @Packs, @Nos, @Discount, @Amount, @GrandTotal)";
