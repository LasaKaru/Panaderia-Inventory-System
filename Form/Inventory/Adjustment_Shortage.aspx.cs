        ...
        protected void btnSave_Click1(object sender, EventArgs e)
        {
            try
            {
                ...
                using (SqlConnection connection = new SqlConnection(connectionString))
                {
                    ...
                    string insertMainQuery = @"INSERT INTO [dbo].[Inv_adjustments]
        ([CompanyID],[IPS_Date],[Branch],[TxnType],[Number],[User],[ref_nu],[Comments],[Amount])
  VALUES
                (@CompanyID, @IPS_Date, @Branch, @TxnType, @Number, @User,  @ref_nu, @Comments,@Amount)";
                    ...
                    using (SqlCommand cmdMain = new SqlCommand(insertMainQuery, connection))
                    {
                        ...
                        cmdMain.Parameters.AddWithValue("@ref_nu", txtReferanceNo.Text);
                        cmdMain.Parameters.AddWithValue("@Comments", txtComments.Text);
                        cmdMain.Parameters.AddWithValue("@Amount", txtTotal.Text);
                    ...
                    foreach (DataRow row in dt.Rows)
                    {
                        string insertDetailQuery = "INSERT INTO adjustments_footer (ItemCode, Description, Price, PSize, Packs, Nos, Discount, Amount, GrandTotal) " +
                ...
                using (PdfWriter writer = new PdfWriter(stream))
                {
                    ...
                    using (PdfDocument pdf = new PdfDocument(writer))
                    {
                        ...
                        using (Document document = new Document(pdf))
                        {
                            ...
                            Paragraph title1 = new Paragraph("Adjustment Shortage Report\n \n ")
                            .SetTextAlignment(TextAlignment.CENTER)
                            .SetFontSize(9);
                            document.Add(title1);
