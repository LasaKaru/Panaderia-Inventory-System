            ...
            using (var stream = new MemoryStream())
            {
                using (var writer = new PdfWriter(stream))
                {
                    using (var pdf = new PdfDocument(writer))
                    {
                        ...
                        Paragraph title1 = new Paragraph("Sales Invoice Report")
                            .SetTextAlignment(TextAlignment.CENTER)
                            .SetFontSize(9);
                        document.Add(title1);
                ...
                File.WriteAllBytes(Server.MapPath("~/Reports/Sales_Invoice_Report.pdf"), stream.ToArray());
            }
        }

        protected void btnDownloadPDF_Click(object sender, EventArgs e)
        {
            string filePath = Server.MapPath("~/Reports/Sales_Invoice_Report.pdf");
            ...
            if (file.Exists)
            {
                ...
                Response.AddHeader("content-disposition", "attachment; filename=Sales_Invoice_Report.pdf");
