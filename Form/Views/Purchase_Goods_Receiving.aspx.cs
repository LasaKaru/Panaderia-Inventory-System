...
{
    public partial class Purchase_Goods_Receiving : System.Web.UI.Page
    {
        ...
        private void GenerateTransactionReport(ref DataTable dataTable)
        {
            ...
            using (SqlConnection connection = new SqlConnection(connectionString))
            {
                using (SqlCommand command = new SqlCommand(query, connection))
                {
                    ...
                    //SendEmailWithAttachments(dataTable);
                }
            }
        }
            ...
            using (var stream = new MemoryStream())
            {
                using (var writer = new PdfWriter(stream))
                {
                    using (var pdf = new PdfDocument(writer))
                    {
                        ...
                        Paragraph title1 = new Paragraph("Purchase Goods Receiving Report")
                            .SetTextAlignment(TextAlignment.CENTER)
                            .SetFontSize(9);
                        document.Add(title1);
                ...
                File.WriteAllBytes(Server.MapPath("~/Reports/Purchase_Goods_Reciving_Report.pdf"), stream.ToArray());
            }
        }

        protected void btnDownloadPDF_Click(object sender, EventArgs e)
        {
            string filePath = Server.MapPath("~/Reports/Purchase_Goods_Reciving_Report.pdf");
            ...
            if (file.Exists)
            {
                ...
                Response.AddHeader("content-disposition", "attachment; filename=Purchase_Goods_Reciving_Report.pdf");
